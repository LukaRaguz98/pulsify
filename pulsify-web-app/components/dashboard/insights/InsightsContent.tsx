...
export function InsightsContent({ guildId, guildName }: Props) {
  ...
  // Plain-language recap — shown on-page and posted verbatim to Discord.
  const recapItems = buildRecap({
    windowDays,
    current: cur,
    trends: overview.trends,
    activeUsers: overview.activeUsers,
    totalMembers: overview.totalMembers,
    peakSlot: bestSlot ? { dow: bestSlot.dow, hour: bestSlot.hour } : null,
    topChannel: data.topChannels[0]?.channel_name ?? null,
  })
  ...
  return (
    <div className="page-content">
      ...
      <div className="space-y-8">
        ...
        {/* ── Weekly recap — turn the analysis into a community post ──────── */}
        <CategorySection
          icon={<Send size={14} />}
          title="Weekly Recap"
          description="A shareable summary of the period — post it straight to your community."
        >
          <RecapCard
            guildId={guildId}
            windowDays={windowDays}
            items={recapItems}
            channels={data.postableChannels}
          />
        </CategorySection>
...
// Documents the Pulse infrastructure: today's cards come from the rule engine
// (Recommendation.source === 'rule'); the same shape will carry AI-authored
// recommendations once Pulse lands.
function EchoAiNote({ guildId }: { guildId: string }) {
  return (
    <div
      className="flex flex-col gap-3 rounded-xl border p-5 sm:flex-row sm:items-center sm:justify-between"
      style={{
        background: 'linear-gradient(120deg, var(--p-soft), transparent)',
        borderColor: 'var(--line-strong)',
      }}
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-foreground">Pulse recommendations are coming</p>
          <p className="mt-0.5 text-xs" style={{ color: 'var(--text-3)' }}>
            Today’s insights are generated by Pulsify’s rules engine. Soon, Pulkse will analyse your
            server in depth and surface even smarter, personalised suggestions right here.
          </p>
        </div>
      </div>
      <Link
