 ...
 * regenerating the whole embed and hoping. Editing, reordering by delete + add,
 * and removing are the minimum for content someone else wrote for you.
 */
export function EmbedFieldsEditor({ fields, onChange, hint }: Props) {
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="block text-xs font-medium text-muted-foreground">
          Cards <span className="text-subtle">({fields.length})</span>
        </label>
        <button
          type="button"
          onClick={add}
      ...
      {fields.length === 0 ? (
        <p className="text-xs text-subtle">
          No cards. The embed is just the title and description.
        </p>
      ) : (
        <div className="space-y-2">
          {fields.map((f, i) => (
