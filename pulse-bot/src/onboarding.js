...
const COMMUNITY_META = {
  guidelines: { emoji: "📜", label: "Community Guidelines" },
  support: { emoji: "🛟", label: "Support" },
  feedback: { emoji: "💬", label: "Feedback" },
  feature_requests: { emoji: "💡", label: "Feature Requests" },
  announcements: { emoji: "📣", label: "Announcements" },
  release_notes: { emoji: "✨", label: "Release Notes" },
};
...
function createOnboarding(client, supabase, leveling = null) {
  ...
  function welcomeHeader(cfg, member, resolve) {
    ...
    let thumbUrl = null;
    if (w.thumbnail === "member_avatar") thumbUrl = member.displayAvatarURL({ extension: "png", size: 128 });
    else if (w.thumbnail === "guild_icon") thumbUrl = member.guild.iconURL({ extension: "png", size: 128 });

    if (thumbUrl) {
      return [{ type: 9, components: lines, accessory: { type: 11, media: { url: thumbUrl } } }];
    }
    // No thumbnail — add a width spacer (unless a banner defines the width).
    if (!w.banner) lines.splice(title ? 2 : 1, 0, { type: 10, content: WIDTH_SPACER });
    return lines;
  }
  ...
  function buildPanel(cfg, member, events) {
    ...
    const quickLinks = (w.quick_links ?? []).filter((q) => q.label && q.channel_id);
    if (quickLinks.length > 0) {
      components.push({
        type: 10,
        content: quickLinks.map((q) => `→ <#${q.channel_id}> — ${q.label}`).join("\n"),
      });
    }

    // Banner image (full width) — referenced from the attached banner.png.
    if (w.banner) {
      components.push({ type: 12, items: [{ media: { url: "attachment://banner.png" } }] });
    }

    const enabledSteps = (cfg.steps ?? []).filter((s) => s.enabled).map((s) => s.id);
    let roleMenus = 0;
      ...
      if (step === "roles") {
        for (const cat of cfg.roleCategories ?? []) {
          ...
          const options = roles.slice(0, 25).map((r) => {
            const o = { label: (r.label || r.role_id).slice(0, 100), value: r.role_id };
            if (r.description) o.description = r.description.slice(0, 100);
            const e = toEmoji(r.emoji);
            if (e) o.emoji = e;
            return o;
          });
          const max = cat.max_select > 0 ? Math.min(cat.max_select, options.length) : options.length;
          components.push({ type: 14, divider: true, spacing: 1 });
          components.push({ type: 10, content: `**${cat.label}**${cat.description ? `\n-# ${cat.description}` : ""}` });
          components.push({
            type: 1,
            components: [{
              type: 3,
              ...
              min_values: Math.max(0, Math.min(cat.min_select ?? 0, options.length)),
        ...
        if (list.length > 0) {
          ...
          const body = list
            .map((e) => {
              const ts = Math.floor(new Date(e.scheduledStartAt ?? e.scheduledStartTimestamp ?? Date.now()).getTime() / 1000);
              return `📅 **${e.name}** — <t:${ts}:R>`;
            })
            .join("\n");
          components.push({ type: 10, content: `**Upcoming events**\n${body}` });
        }
      } else if (step === "community") {
        const links = COMMUNITY_ORDER
          .filter((k) => cfg.community?.[k])
          .map((k) => `${COMMUNITY_META[k].emoji} ${COMMUNITY_META[k].label} → <#${cfg.community[k]}>`);
        if (links.length > 0) {
          components.push({ type: 14, divider: true, spacing: 1 });
          components.push({ type: 10, content: `**Get to know ${member.guild.name}**\n${links.join("\n")}` });
        }
      } else if (step === "rewards" && cfg.rewards?.enabled) {
        const parts = [];
        if (cfg.rewards.xp) parts.push(`**${cfg.rewards.xp} XP**`);
        if (cfg.rewards.reputation) parts.push(`**+${cfg.rewards.reputation} reputation**`);
        if ((cfg.rewards.role_ids ?? []).length) parts.push(`**${cfg.rewards.role_ids.length} starter role${cfg.rewards.role_ids.length === 1 ? "" : "s"}**`);
        if (parts.length) {
          components.push({ type: 14, divider: true, spacing: 1 });
          components.push({ type: 10, content: `🎁 Finish onboarding to earn ${parts.join(" · ")}.` });
        }
      }
    }

    // Action buttons: verify, link buttons, finish.
    const buttonRow = [];
    if (enabledSteps.includes("verification") && cfg.verification?.enabled && cfg.verification.role_id) {
      buttonRow.push({
        type: 2, style: 3,
    ...
    if (buttonRow.length > 0) {
      components.push({ type: 14, divider: true, spacing: 1 });
      components.push({ type: 1, components: buttonRow.slice(0, 5) });
    }

    // External link buttons (their own rows; 5 per row).
    const linkButtons = (w.buttons ?? []).filter((b) => b.label && b.url);
    for (let i = 0; i < linkButtons.length; i += 5) {
      components.push({
        type: 1,
        components: linkButtons.slice(i, i + 5).map((b) => {
    ...
    // Footer.
    components.push({ type: 14, divider: true, spacing: 1 });
    components.push({ type: 10, content: `-# ${w.footer_text ? resolve(w.footer_text) : "Pulse · Welcome"}` });
  ...
  async function postForMember(member, settings) {
    try {
      ...
      let events = [];
      const eventsOn = (cfg.steps ?? []).some((s) => s.id === "events" && s.enabled) && cfg.events?.enabled;
      if (eventsOn) {
        try {
      ...
      console.error(`[Pulse] Onboarding post failed in guild ${member.guild.id}:`, err.message);
    }
  }
  ...
  async function handleRole(interaction, guildId, catId) {
    ...
    if (!ctx) return interaction.reply({ content: "Onboarding is no longer available.", flags: MessageFlags.Ephemeral });
    const cat = (ctx.cfg.roleCategories ?? []).find((c) => c.id === catId);
    if (!cat) return interaction.reply({ content: "That role group no longer exists.", flags: MessageFlags.Ephemeral });

    const categoryRoleIds = (cat.roles ?? []).map((r) => r.role_id).filter(Boolean);
    const selected = new Set(interaction.values);
    const toAdd = [...selected].filter((id) => !ctx.member.roles.cache.has(id));
    const toRemove = categoryRoleIds.filter((id) => !selected.has(id) && ctx.member.roles.cache.has(id));

    try {
      if (toAdd.length) await ctx.member.roles.add(toAdd, "Pulse onboarding self-roles").catch(() => {});
      if (toRemove.length) await ctx.member.roles.remove(toRemove, "Pulse onboarding self-roles").catch(() => {});
    } catch { /* role above the bot / perms — best effort */ }
    ...
    const prevSelected = new Set(Array.isArray(prev?.selected_roles) ? prev.selected_roles : []);
    for (const id of categoryRoleIds) prevSelected.delete(id);
    for (const id of selected) prevSelected.add(id);
    const prevSkipped = (Array.isArray(prev?.skipped_steps) ? prev.skipped_steps : []).filter((s) => s !== "roles");
    ...
    await interaction.reply({
      content: selected.size ? `✅ Updated your **${cat.label}** roles.` : `Cleared your **${cat.label}** roles.`,
  ...
  async function handleVerify(interaction, guildId) {
    ...
    if (!ctx.cfg.verification?.enabled || !roleId)
      return interaction.reply({ content: "Verification isn't enabled here.", flags: MessageFlags.Ephemeral });

    try {
      await ctx.member.roles.add(roleId, "Pulse onboarding verification");
    } catch {
      return interaction.reply({ content: "I couldn't assign the verified role — please ping a moderator.", flags: MessageFlags.Ephemeral });
    }

    const prev = await readProgress(guildId, interaction.user.id);
    const prevSkipped = (Array.isArray(prev?.skipped_steps) ? prev.skipped_steps : []).filter((s) => s !== "verification");
    await supabase.from("onboarding_member_progress").upsert(
      {
        guild_id: guildId,
        user_id: interaction.user.id,
    ...
    await interaction.reply({
      content: ctx.cfg.verification.success_message || "You're verified — welcome aboard! ✅",
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  async function handleDone(interaction, guildId) {
    ...
    const prev = await readProgress(guildId, interaction.user.id);
    if (prev?.rewarded)
      return interaction.reply({ content: "You've already completed onboarding — welcome back! 👋", flags: MessageFlags.Ephemeral });

    const enabled = (ctx.cfg.steps ?? []).filter((s) => s.enabled).map((s) => s.id);
    const selectedRoles = Array.isArray(prev?.selected_roles) ? prev.selected_roles : [];
    const verified = !!prev?.verified;

    // A step the member never engaged counts as skipped (roles / verification).
    const skipped = [];
    if (enabled.includes("roles") && selectedRoles.length === 0) skipped.push("roles");
    if (enabled.includes("verification") && ctx.cfg.verification?.enabled && !verified) skipped.push("verification");
    ...
    if (rewards.enabled) {
      if (rewards.xp > 0) {
        try {
          ...
          if (!error) { xpAwarded = Math.floor(rewards.xp); rewardLines.push(`**${xpAwarded} XP**`); }
        } catch { /* leveling table missing — skip */ }
      }
      for (const roleId of rewards.role_ids ?? []) {
        await ctx.member.roles.add(roleId, "Pulse onboarding completion reward").catch(() => {});
      }
      if ((rewards.role_ids ?? []).length) rewardLines.push(`**${rewards.role_ids.length} starter role${rewards.role_ids.length === 1 ? "" : "s"}**`);
      if (rewards.reputation > 0) rewardLines.push(`**+${rewards.reputation} reputation**`);
    }
    ...
    await supabase.from("onboarding_member_progress").upsert(
      {
        ...
        reputation_bonus: rewards.enabled ? Math.floor(rewards.reputation || 0) : 0,
  ...
  async function onInteraction(interaction) {
    try {
      ...
      if (!guildId) return;
      if (action === "role" && isSelect) return handleRole(interaction, guildId, arg);
      if (action === "verify" && isButton) return handleVerify(interaction, guildId);
      if (action === "done" && isButton) return handleDone(interaction, guildId);
    } catch (err) {
