...
create policy "Service role full access to guild_settings" on public.guild_settings for all using (true)
with
    check (true);
...
create index if not exists guild_warnings_guild_user on public.guild_warnings (guild_id, user_id);

alter table public.guild_warnings enable row level security;

create policy "Service role full access to guild_warnings" on public.guild_warnings for all using (true)
with
    check (true);
...
create policy "Service role full access to synced_guilds" on public.synced_guilds for all using (true)
with
    check (true);
/